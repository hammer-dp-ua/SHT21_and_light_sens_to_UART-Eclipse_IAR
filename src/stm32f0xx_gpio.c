  ...
  for (pinpos = 0x00; pinpos < 0x10; pinpos++)
  {
    ...
    if (currentpin == pos)
    {
      if ((GPIO_InitStruct->GPIO_Mode == GPIO_Mode_OUT) || (GPIO_InitStruct->GPIO_Mode == GPIO_Mode_AF))
      {
        ...
        GPIOx->OSPEEDR |= ((unsigned int)(GPIO_InitStruct->GPIO_Speed) << (pinpos * 2));
        ...
        GPIOx->OTYPER |= (unsigned short)(((unsigned short)GPIO_InitStruct->GPIO_OType) << ((unsigned short)pinpos));
      }

      GPIOx->MODER  &= ~(GPIO_MODER_MODER0 << (pinpos * 2));

      GPIOx->MODER |= (((unsigned int)GPIO_InitStruct->GPIO_Mode) << (pinpos * 2));

      /* Pull-up Pull down resistor configuration */
      GPIOx->PUPDR &= ~(GPIO_PUPDR_PUPDR0 << ((unsigned short)pinpos * 2));
      GPIOx->PUPDR |= (((unsigned int)GPIO_InitStruct->GPIO_PuPd) << (pinpos * 2));
    }
  }
}
...
{
  __IO unsigned int tmp = 0x00010000;
...
unsigned char GPIO_ReadInputDataBit(GPIO_TypeDef* GPIOx, unsigned short GPIO_Pin)
{
  ...
  if ((GPIOx->IDR & GPIO_Pin) != (unsigned int)Bit_RESET)
  {
    bitstatus = (unsigned char)Bit_SET;
  }
  else
  {
    bitstatus = (unsigned char)Bit_RESET;
  }
  return bitstatus;
}
...
/*unsigned short GPIO_ReadInputData(GPIO_TypeDef* GPIOx)
{
  ...
  return ((unsigned short)GPIOx->IDR);
}*/
...
unsigned char GPIO_ReadOutputDataBit(GPIO_TypeDef* GPIOx, unsigned short GPIO_Pin)
{
  ...
  if ((GPIOx->ODR & GPIO_Pin) != (unsigned int)Bit_RESET)
  {
    bitstatus = (unsigned char)Bit_SET;
  }
  else
...
/*unsigned short GPIO_ReadOutputData(GPIO_TypeDef* GPIOx)
{
  ...
  return ((unsigned short)GPIOx->ODR);
}*/
