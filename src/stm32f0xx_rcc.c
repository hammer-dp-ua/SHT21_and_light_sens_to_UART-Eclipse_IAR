...
/* ---------------------- RCC registers mask -------------------------------- */
/* RCC Flag Mask */
#define FLAG_MASK                 ((unsigned char)0x1F)

/* CR register byte 2 (Bits[23:16]) base address */
#define CR_BYTE2_ADDRESS          ((unsigned int)0x40021002)

/* CFGR register byte 3 (Bits[31:23]) base address */
#define CFGR_BYTE3_ADDRESS        ((unsigned int)0x40021007)

/* CIR register byte 1 (Bits[15:8]) base address */
#define CIR_BYTE1_ADDRESS         ((unsigned int)0x40021009)

/* CIR register byte 2 (Bits[23:16]) base address */
#define CIR_BYTE2_ADDRESS         ((unsigned int)0x4002100A)

/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static __I unsigned char APBAHBPrescTable[16] = {0, 0, 0, 0, 1, 2, 3, 4, 1, 2, 3, 4, 6, 7, 8, 9};
...
/*void RCC_DeInit(void)
{
  ...
  RCC->CFGR &= (unsigned int)0xF8FFB80C;
#else
  // Reset SW[1:0], HPRE[3:0], PPRE[2:0], ADCPRE, MCOSEL[2:0], MCOPRE[2:0] and PLLNODIV bits
  RCC->CFGR &= (unsigned int)0x08FFB80C;
#endif // STM32F051
  ...
  RCC->CFGR &= (unsigned int)0xFFC0FFFF;

  // Reset PREDIV1[3:0] bits
  RCC->CFGR2 &= (unsigned int)0xFFFFFFF0;

  // Reset USARTSW[1:0], I2CSW, CECSW and ADCSW bits
  RCC->CFGR3 &= (unsigned int)0xFFF0FEAC;
  
  // Reset HSI14 bit
  RCC->CR2 &= (unsigned int)0xFFFFFFFE;
...
/*void RCC_HSEConfig(unsigned char RCC_HSE)
{
  ...
  *(__IO unsigned char *) CR_BYTE2_ADDRESS = RCC_HSE_OFF;

  // Set the new HSE configuration -------------------------------------------
  *(__IO unsigned char *) CR_BYTE2_ADDRESS = RCC_HSE;
...
{
  __IO unsigned int StartUpCounter = 0;
...
/*void RCC_AdjustHSICalibrationValue(unsigned char HSICalibrationValue)
{
  ...
  tmpreg |= (unsigned int)HSICalibrationValue << 3;
...
/*void RCC_AdjustHSI14CalibrationValue(unsigned char HSI14CalibrationValue)
{
  ...
  tmpreg |= (unsigned int)HSI14CalibrationValue << 3;
...
void RCC_PLLConfig(unsigned int RCC_PLLSource, unsigned int RCC_PLLMul)
{
  ...
  RCC->CFGR |= (unsigned int)(RCC_PLLSource | RCC_PLLMul);
}
...
/*void RCC_MCOConfig(unsigned char RCC_MCOSource)
{
  ...
  *(__IO unsigned char *) CFGR_BYTE3_ADDRESS =  RCC_MCOSource;
}*/
#else
...
/*void RCC_MCOConfig(unsigned char RCC_MCOSource, unsigned int RCC_MCOPrescaler)
{
  ...
  tmpreg |= (RCC_MCOPrescaler | ((unsigned int)RCC_MCOSource<<24));
  // Store the new value
  RCC->CFGR = tmpreg;
}*/
#endif /* STM32F072 */
...
{
  return ((unsigned char)(RCC->CFGR & RCC_CFGR_SWS));
}*/
...
/**
  ...
  * @retval None
  */
/*void RCC_ADCCLKConfig(unsigned int RCC_ADCCLK)
{ 
  // Check the parameters
...
{ 
  ...
  if (tmp == (unsigned int)0x00000001)
  {
    // Clear USART1SW[1:0] bit
    RCC->CFGR3 &= ~RCC_CFGR3_USART1SW;
  }
  else if (tmp == (unsigned int)0x00000002)
  {
...
/**
  ...
  * @retval None
  */
/*void RCC_USBCLKConfig(unsigned int RCC_USBCLK)
{ 
  // Check the parameters
...
{
  unsigned int tmp = 0, pllmull = 0, pllsource = 0, prediv1factor = 0, presc = 0, pllclk = 0;
...
/*void RCC_ITConfig(unsigned char RCC_IT, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    *(__IO unsigned char *) CIR_BYTE1_ADDRESS |= RCC_IT;
  }
  else
  {
    // Perform Byte access to RCC_CIR[13:8] bits to disable the selected interrupts
    *(__IO unsigned char *) CIR_BYTE1_ADDRESS &= (unsigned char)~RCC_IT;
  }
}*/
...
FlagStatus RCC_GetFlagStatus(unsigned char RCC_FLAG)
{
  ...
  if ((statusreg & ((unsigned int)1 << tmp)) != (unsigned int)RESET)
  {
    bitstatus = SET;
  }
  else
...
/*ITStatus RCC_GetITStatus(unsigned char RCC_IT)
{
  ...
  if ((RCC->CIR & RCC_IT) != (unsigned int)RESET)
  {
    bitstatus = SET;
  }
  else
...
/*void RCC_ClearITPendingBit(unsigned char RCC_IT)
{
  ...
  *(__IO unsigned char *) CIR_BYTE2_ADDRESS = RCC_IT;
}*/
