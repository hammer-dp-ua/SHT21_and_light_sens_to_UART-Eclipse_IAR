...
/*void SYSCFG_DeInit(void)
{
  ...
  SYSCFG->CFGR2 |= (unsigned int) SYSCFG_CFGR2_SRAM_PE;
}*/
...
/*void SYSCFG_MemoryRemapConfig(unsigned int SYSCFG_MemoryRemap)
{
  ...
  // Clear MEM_MODE bits 
  tmpctrl &= (unsigned int) (~SYSCFG_CFGR1_MEM_MODE);

  // Set the new MEM_MODE bits value 
  tmpctrl |= (unsigned int) SYSCFG_MemoryRemap;
...
/*void SYSCFG_DMAChannelRemapConfig(unsigned int SYSCFG_DMARemap, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    SYSCFG->CFGR1 |= (unsigned int)SYSCFG_DMARemap;
  }
  else
  {
...
/*void SYSCFG_I2CFastModePlusConfig(unsigned int SYSCFG_I2CFastModePlus, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    SYSCFG->CFGR1 |= (unsigned int)SYSCFG_I2CFastModePlus;
  }
  else
  {
...
void SYSCFG_EXTILineConfig(unsigned char EXTI_PortSourceGPIOx, unsigned char EXTI_PinSourcex)
{
  ...
  tmp = ((unsigned int)0x0F) << (0x04 * (EXTI_PinSourcex & (unsigned char)0x03));
  SYSCFG->EXTICR[EXTI_PinSourcex >> 0x02] &= ~tmp;
  SYSCFG->EXTICR[EXTI_PinSourcex >> 0x02] |= (((unsigned int)EXTI_PortSourceGPIOx) << (0x04 * (EXTI_PinSourcex & (unsigned char)0x03)));
}
...
/*void SYSCFG_BreakConfig(unsigned int SYSCFG_Break)
{
  ...
  SYSCFG->CFGR2 |= (unsigned int) SYSCFG_Break;
}*/
...
/*FlagStatus SYSCFG_GetFlagStatus(unsigned int SYSCFG_Flag)
{
  ...
  if ((SYSCFG->CFGR2 & SYSCFG_CFGR2_SRAM_PE) != (unsigned int)RESET)
  {
    // SYSCFG_Flag is set 
    bitstatus = SET;
  }
...
/*void SYSCFG_ClearFlag(unsigned int SYSCFG_Flag)
{
  ...
  SYSCFG->CFGR2 |= (unsigned int) SYSCFG_Flag;
}*/
